...
describe("middleware integration with wrapLanguageModel", () => {
  describe("logging middleware", () => {
    it("logs generate calls with timing and usage", async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith("ai.generate.start", expect.objectContaining({
        model: "mock-model",
      }));
      expect(logger.debug).toHaveBeenCalledWith("ai.generate.complete", expect.objectContaining({
  ...
  describe("observability middleware", () => {
    it("records generation with trace metadata", async () => {
      ...
      expect(observability.generation).toHaveBeenCalledWith(expect.objectContaining({
        traceId: "trace-123",
        name: "analyzer",
        output: "AI response",
        metadata: { userId: "user-1" },
