...
`npm test`. The SUFFIX says the kind and the folder follows it, and the fork is
`@jterrazz/test`'s, not a preference of this repository.

| Project       | Collects                                      | Proves                                                     |
| ------------- | --------------------------------------------- | ---------------------------------------------------------- |
| `unit`        | `<file>.test.ts` beside `<file>.ts`           | one module alone, against doubles it builds itself         |
| `integration` | `specs/integration/<domain>/<aspect>.spec.ts` | a module whose oracle is a golden, or that needs a process |
...
`specs/integration/pipeline/composition.spec.ts` wraps a REAL
...
`specs/integration/formatting/to-sentence-case.spec.ts` is fifty-four
...
`specs/integration/lint/rules.spec.ts` executes the REAL `oxlint` binary
...
`oxlint.baseline.json` records 210 diagnostics across 10 rules, five of which
...
- `vitest/max-nested-describe`, 18 — the five module tests that nest
  `describe('<method>')` inside `describe('<subject>')`.
- `jterrazz/i4-no-module-doubles`, 2 — the `vi.mock` calls that stand in for
